...
class SqliteMemoryStore implements MemoryStore {
  ...
  /// BM25 saturation pivot used to normalize FTS5 scores into [0, 1] as
  /// `bm25 / (bm25 + k)`. Smaller k pushes scores toward 1 faster. The
  /// default suits short memory sentences, where a solid single-term
  /// match lands around bm25 ≈ 1–3 (→ 0.5–0.75 normalized).
  ...
  SqliteMemoryStore(this._db, {String prefix = '', this.bm25SaturationK = 1.0})
      : _prefix = prefix;
  ...
  Future<void> update(
    ...
    if (content != null) {
      sets.add('content = ?');
      params.add(content);
      // Content changed: a stale vector must not survive. Cleared here and
      // re-generated by the engine's post-consolidation embedding pass,
      // unless the caller supplied a fresh embedding below.
      if (embedding == null) {
        sets.add('embedding = NULL');
      }
    }
    if (importance != null) {
      sets.add('importance = ?');
